...
const getAllContainersName = async () => {
    input = spawn('curl', ['-v', '--unix-socket', '/var/run/docker.sock', `http://localhost/containers/json`]);
    ...
    return new Promise((resolve, reject) => {
        ...
        input.stdout.on('end', () => {
            ...
            resolve(containers.reduce((acc, { Names }) => acc.concat(Names.map(name => name.replace('/', ''))), []));
        });
    });
};
